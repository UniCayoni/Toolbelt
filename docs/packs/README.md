...
| **Routers / pocket entry** | shipped (Themes 14 + 17) | De facto: `research-scope`, `design-process`. **`implementation-router`**, **`debug-router`** (+ templates). Happy-path **chains** these. Global meta-router **parked**. See Theme 14 / 17 reports |
...
| **Host standards** | shipped (Theme 16) | **`author-standards`** + templates `principles-profile` / `standards-profile` / `author-standards-checklist`. Plan/Execute/Closeout bind when present. **Not** Toolbelt-universal coding law. See `docs/research/reports/theme-16-host-standards.md` |
