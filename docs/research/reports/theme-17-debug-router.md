...
aligned_with:
  - docs/research/notes/theme-17-debug-router/campaign-brief.md
  - docs/research/notes/theme-17-debug-router/pre-start-considerations.md
  - docs/research/notes/theme-17-debug-router/t17a-local-baseline.md
  - docs/research/notes/theme-17-debug-router/t17b-classifier-matrix.md
  - docs/research/notes/theme-17-debug-router/t17c-e-shape-wire-smoke-lean.md
  - docs/research/reports/theme-14-pocket-routers.md
  - docs/research/reports/theme-9-debug-pocket.md
supersedes: null
amends:
  - docs/research/reports/theme-14-pocket-routers.md  # D4: defer debug-router → shipped
  - docs/research/reports/theme-10-happy-path.md  # Debug stage → debug-router
---

# Theme 17 — Debug router

**Status:** **accepted** (method guidance) — 2026-08-02.  
**Elevated:** via `/author-cursor-surfaces` — `debug-router` + template; happy-path / impl-router wire; Execute repro-first note.  
...
Complete the Theme 14 router layer for Debug: ship **`debug-router`** as compose-only classify + wire over Theme 9 leaves. Happy-path and `implementation-router` hand off here; Execute keeps a direct-leaf hot path with a repro-first rule.

## 2. Elevation decisions (accepted)

| # | Decision |
|---|----------|
| D1 | Theme id **`theme-17-debug-router`**; amends Theme 14 **D4** (defer → ship) |
| D2 | Skill **`debug-router`**; discoverable (no `disable-model-invocation`) |
| D3 | Template **`docs/templates/debug-router.md`** (+ refresh → skill references) |
...
| D7 | **Seams:** happy-path + impl-router → `debug-router`; Execute/-verify/-subagents → **direct leaf OK** + repro-first (no solid repro → reproduce; repro in hand → systematic; router when path unclear) |
...
| Pocket | Router / entry |
|--------|----------------|
| Research | `research-scope` (de facto) |
| Design | `design-process` (de facto) |
| Implementation | `implementation-router` |
| Debug | **`debug-router`** |
