# Handoff — I1 fresh chat (`implementation-router`)
...
**Theme 14 smoke I1** — skill `implementation-router`.  
...
> Using `implementation-router`: assume design for a smoke-app `add()` off-by-one fix is already human-accepted (treat as given; do not invent new product locks). Wire the Implementation pocket only. Output: announce, classifier, structured handoff block, wire plan with explicit N/A skips. Do not write plan body. Do not implement.

### Part B — verify-only (required)

> Using `implementation-router`: Meta `ready` plan already exists and task greens are done. Need EOP `implementation-execute-verify` only. Wire with skips. Do not implement.

### Part C — single leaf / skip (required)

> Using `implementation-router`: only run `implementation-plan-verify` on the existing plan. Either document router skip + leaf-direct, or emit a thin wire that is verify-only. Do not force plan→execute.

### Pass bar (quick)

- Announces **Using `implementation-router`**
