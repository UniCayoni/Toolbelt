...
Next skill for code-level shape / boundaries if needed: **`design-technical`** (trivial one-file fix may then skip durable plan per guide-design intelligent exception — still **human gate first**).

After accept, implementation pocket only if human directs: **`guide-implementation`** (or leaves: `implementation-plan` → …). Trivial one-file exception may apply. **Do not implement in this pass.**
