...
description: >-
  ...
  or systematic-debug. Prefer reproduce-bug first when the job is prove-only.
  Not Theme 8 plan/execute-verify; not PR/CI packaging.
---
...
**Prefer first:** **`reproduce-bug`** when the ask is prove/minimize/dossier only (never-fix).
...
2. **Reproduce** — same surface as the report; shrink to load-bearing steps  
   ...
   - Cannot repro → `NOT-YET-REPRODUCED` (+ light dossier via `reproduce-bug` / `repro-light`); no guess-fix
...
Read `references/systematic-debug-checklist.md` **when** running a full session or recovering a skipped step.
...
| T-NYR | Need prove-first → `reproduce-bug` |
...
| Need | Use |
|------|-----|
| Prove / dossier only (never-fix) | **`reproduce-bug`** |
...
| Unfamiliar code before investigate | `codebase-recon` (as-needed) |
| PR / CI / Bugbot | Phase 2 — not this skill |

## References

- Read `references/systematic-debug-checklist.md` **when** running a full debug session
- Light dossier fields: Toolbelt `docs/templates/repro-light.md` / skill `reproduce-bug`
