...
**Out of scope:** Writing/revising the plan (→ `implementation-plan`); graded plan-validate (→ `implementation-plan-verify`); Design options; Build-domain cookbooks; mandatory git/worktree/TDD/PR packaging; Debug method (→ `debug-systematic` / `debug-reproduce`); PR/CI packaging.
...
3. **Task loop** (`exec_default: serial_implement_review` unless plan marks parallel-safe)
   ...
   - On mismatch → local fix inside Files/Interfaces (enhanced, not blind retry), up to **N=2** attempts (**verify-retry**); then `blocked`+`verify-fail` → hand off **`debug-systematic`** (or **`debug-reproduce`** if prove-first). Do **not** burn more verify-retries under Debug.
