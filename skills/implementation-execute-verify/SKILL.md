...
description: >-
  ...
  for plan writing or PR/Debug packaging.
---
...
**Identity:** Verification **extension of Execute** — not the Debug/PR pack. Quality + code **readability** + faithfulness to plan/design first; ease is a side effect.
...
**Out of scope:** Plan authoring/validate (`implementation-plan` / `implementation-plan-verify`); Debug/PR/git/merge; re-litigating N=2; foreign CLI deps; silent Goal rewrite.
...
Routing: Critical → fix + re-evidence (or Theme 7 major-deviation); Important → fix before proceed; Minor → note. HITL = Theme 7 only.
...
| PR / debug / merge | Later Debug/PR pack — not here |
