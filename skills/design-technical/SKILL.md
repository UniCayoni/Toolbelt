...
5. **ADR triggers** — draft via **`research-draft-adr`** when the choice affects structure, NFRs, dependencies, interfaces, or construction techniques, or when ≥2 real options exist / the decision would otherwise be undocumented.
...
- After decide → `/research-draft-adr` with Considered Options
...
- Read `references/design-technical-checklist.md` **when** doing a full architecture pass
- ADRs: skill `research-draft-adr` → `docs/adr/`
