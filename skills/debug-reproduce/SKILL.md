...
description: >-
  ...
  any product patch. Use when debug-reproduce, prove the bug, minimal repro,
  flaky/intermittent, NOT-YET-REPRODUCED, or hand a fixer a regression guard.
  Does not implement fixes — hand off to debug-systematic. Not Theme 8 verify;
  not PR packaging. Formerly reproduce-bug.
---

# Reproduce bug

Announce once: **Using `debug-reproduce`**.
...
**Identity:** Never-fix **companion** to `debug-systematic`. Quality = honest failing repro + cold handoff.
...
- Handoff to a fixer or to `debug-systematic` for the fix phase
...
7. **Handoff** — point fixer at command + acceptance (“green when this repro passes”); for product fix use **`debug-systematic`**

Read `references/debug-reproduce-checklist.md` **when** running a full prove session.  
